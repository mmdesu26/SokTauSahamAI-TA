...
def get_stocks():
    ...
    for item in items:
        ...
        try:
            quote = YFinanceHelper.get_latest_quote(f"{item.ticker}.JK") or {}
...
def get_stock_detail_full(ticker):
    try:
        ...
        if timeframe == "1D":
            intraday = YFinanceHelper.get_intraday_ohlc(ticker, interval="60m", period="5d") or {}
        ...
        log_external_service(
            "Stock Detail",
            f"Detail saham gagal untuk {ticker.upper()}",
            details=str(e),
            user_id=None,
...
def get_candlestick_data(ticker):
    try:
        ...
        if timeframe == "1D":
            ...
            historical = YFinanceHelper.get_range_ohlc(ticker, timeframe=timeframe) or {}
        ...
        log_external_service(
            "Candlestick",
            f"Candlestick gagal untuk {ticker.upper()}",
            details=str(e),
            user_id=None,
            ip_address=_request_ip()
        )
...
def get_stock_prediction(ticker):
    ...
    try:
        ...
        base_date = request.args.get("base_date") or request.args.get("cutoff_date")
        days = request.args.get("days", default=730, type=int)
        forecast_horizon = request.args.get("forecast_horizon", default=1, type=int)
        lag_days = request.args.get("lag_days", default=15, type=int)
        ...
        log_external_service(
            "ML Prediction",
            f"Prediksi gagal untuk {ticker.upper()}",
            details=str(e),
            user_id=None,
...
def get_stock_fundamentals(ticker):
    ...
    fundamentals = YFinanceHelper.get_fundamentals(f"{ticker.upper()}.JK") or {}
    ...
    return jsonify({
        "success": True,
        "data": {
            "profile": profile_row.to_dict() if profile_row else None,
            "fundamentals": {
                "ratios": {
